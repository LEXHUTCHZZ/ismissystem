          ...
          {allStudents.length > 0 ? (
            allStudents.map((s) => (
              <div key={s.id} style={{ marginBottom: "1.5rem", border: "1px solid #7F1D1D", padding: "1rem", borderRadius: "4px" }}>
                ...
                <p style={{ color: "#7F1D1D", marginTop: "0.5rem" }}><strong>Total Owed:</strong> {s.totalOwed?.toLocaleString() || "0"} JMD</p>
                <p style={{ color: "#7F1D1D" }}><strong>Total Paid:</strong> {s.totalPaid?.toLocaleString() || "0"} JMD</p>
                <p style={{ color: "#7F1D1D" }}><strong>Balance:</strong> {s.balance?.toLocaleString() || "0"} JMD</p>
                <p style={{ color: "#7F1D1D" }}><strong>Payment Status:</strong> {s.paymentStatus || "N/A"}</p>
              </div>
            ))
          ) : (
